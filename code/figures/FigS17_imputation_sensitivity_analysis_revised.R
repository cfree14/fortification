...
gstats1 <- gstats %>% 
  ...
  mutate(change=ifelse(ndeficient_delta<0, "Fewer", "More"))
...
g1 <- ggplot(gstats1, aes(y=tidytext::reorder_within(nutrient, ndeficient_delta, scenario),
  ...
  # Labels
  labs(x="Δ millions of people with indadequate intakes\nwhen ignoring proxy values for fortification compliance", y="", tag="A") +
  # Legend
  scale_fill_manual(name="Change in adequacies", values=c("blue", "red")) +
  # Theme
  theme_bw() + base_theme +
...
# Export
ggsave(g, filename=file.path(plotdir, "FigS17_imputation_sensitivity_analysis_revised.png"), 
