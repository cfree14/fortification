...
# THIS SCRIPT IS IDENTICAL TO THE OTHER EXCEPT THE LINES ADDED THAT ARE FLAGGED, IF YOU UUPDATE THE OTHER, 
# REMAKE IT BY JUST ADDING THESE LINE
...
subs <- scen_data %>%
  ...
  mutate(subsidy_mg1=now_on*daily_intake_kg*processed_prop*fortified_prop_no_proxies*standard_mg_kg/bioavail_scalar) %>% 
  #### NO PROXIES ADDTION
  # Calculate Scenario 2 (improved compliance) subsidy
  mutate(fortified_prop2=pmax(fortified_prop_no_proxies, 0.9)) %>% 
  ...
  mutate(subsidy_mg3=now_on*daily_intake_kg*processed_prop*fortified_prop_no_proxies*aligned_standard_mg_kg_use/bioavail_scalar) %>% 
...
data1_fort_ln <- data1_fort %>% 
  ...
  mutate(sev1=nutriR::sev(ear = ar, cv = ar_cv, meanlog=ln_meanlog1, sdlog=ln_sdlog1, plot=F),
         ...
         sev5=nutriR::sev(ear = ar, cv = ar_cv, meanlog=ln_meanlog5, sdlog=ln_sdlog5, plot=F)) %>%
  ungroup() %>%
...
data1_fort_g <- data1_fort %>% 
  ...
  mutate(sev1=nutriR::sev(ear = ar, cv = ar_cv, shape=g_shape1, rate=g_rate1, plot=F),
         ...
         sev5=nutriR::sev(ear = ar, cv = ar_cv, shape=g_shape5, rate=g_rate5, plot=F)) %>%
  ungroup() %>%
...
# Export data
saveRDS(data2, file=file.path(outdir, "fortification_scenario_output_imputation_sens.Rds"))
