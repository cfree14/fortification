...
data <-  iso3_fv_grid %>% 
  ...
  rename(fortified_prop_source_type=fortified_prop_type) %>% 
  ...
  mutate(fortified_prop_use=case_when(fortified_prop_type == "Imputed" & !is.na(fortified_prop_region) ~ fortified_prop_region,  
                                      ...
                                      fortified_prop_type == "Imputed" & is.na(fortified_prop_region) & is.na(fortified_prop_continent) & !is.na(fortified_prop_income) ~ fortified_prop_income, 
                                      T ~ fortified_prop)) %>% 
  ...
  ## PERCENT FORTIFIED (NO PROXIES)
  # Erase fortified props from proxies
  mutate(fortified_prop_no_proxies=ifelse(grepl("Proxy", fortified_prop_source_type) & !is.na(fortified_prop_source_type), NA, fortified_prop)) %>% 
  # Record imputation status
  ...
  ungroup() %>% 
  # Calculate % fortified by region only
  group_by(continent, food_vehicle) %>% 
  mutate(fortified_prop_no_proxies_region_only=mean(fortified_prop_no_proxies, na.rm=T)) %>% 
  ungroup() %>% 
  # Pick best % fortified based on hierarchy
  mutate(fortified_prop_no_proxies_use=case_when(fortified_prop_no_proxies_type == "Imputed" & !is.na(fortified_prop_no_proxies_region) ~ fortified_prop_no_proxies_region,  
                                                 ...
                                                 fortified_prop_no_proxies_type == "Imputed" & is.na(fortified_prop_no_proxies_region) & is.na(fortified_prop_no_proxies_continent) & !is.na(fortified_prop_no_proxies_income) ~ fortified_prop_no_proxies_income, 
                                                 fortified_prop_no_proxies_type == "Imputed" & is.na(fortified_prop_no_proxies_region) & is.na(fortified_prop_no_proxies_continent) & is.na(fortified_prop_no_proxies_income) & !is.na(fortified_prop_no_proxies_region_only) ~ fortified_prop_no_proxies_region_only, 
  ...
  select(continent, region, income, country, iso3,
         ...
         fortified_prop_type, year_pfort, fortified_prop_source_type,
         fortified_prop_use, fortified_prop, fortified_prop_region, fortified_prop_continent, fortified_prop_income,
         fortified_prop_no_proxies_type, fortified_prop_no_proxies,
...
data_simple <- data %>% 
  ...
  select(continent, region, income, country, iso3,
         ...
         fortified_prop_type, fortified_prop_use, 
         fortified_prop_no_proxies_type, fortified_prop_no_proxies_use) %>% 
  # Rename
...
ggplot(data_simple, aes(x=fortified_prop, y=fortified_prop_no_proxies, color=fortified_prop_no_proxies_type)) + 
  geom_point()
...
data_plot <- data %>% 
  select(iso3, food_vehicle, fort_status, fortified_prop:fortified_prop_income) %>% 
  gather(key="imputation_type", value="fortified_prop_imputed", 5:ncol(.))

ggplot(data_plot, aes(x=fortified_prop, y=fortified_prop_imputed)) +
  facet_grid(food_vehicle~imputation_type) +
  #
