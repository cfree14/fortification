...
subs <- scen_data %>%
  ...
  ################### CALCIUM SENSITIVITY ANANLYS ADDITION ONLY ################### 
  mutate(aligned_standard_mg_kg_use=ifelse(nutrient=="Calcium" & food_vehicle=="Wheat flour", standard_mg_kg, aligned_standard_mg_kg_use)) %>%
  ################### CALCIUM SENSITIVITY ANANLYS ADDITION ONLY ################### 
...
# Export data
saveRDS(data2, file=file.path(outdir, "fortification_scenario_output_calcium.Rds"))
